...
type StorageSpec struct {
	...
	// container runs with: the controller states the path it mounts at as HOME on
	// the main container, so where the workspace is and where the container writes
	...
	// handed serves the workspace. A declared HOME that this outranks is reported on
...
type RuntimeSpec struct {
	...
	// workspace without having to know what its own image bakes. An environment
	// without a claim states no home and keeps the image's.
	//
