...
type SSHSpec struct {
	...
	// Secret's data[key] (default "keys") as the container's authorized_keys, so
	// the Secret must carry the label ai.cubestack.io/ssh-keys-delegated: "true"
...
type DevEnvironmentStatus struct {
	...
	// /run/ssh/authorized_keys; in the generated case the same Secret also carries
	// the private key to log in with, as id_ed25519. The environment's host key
	// is not here: it lives in the separate <env>-ssh-host-key Secret.
	// +optional
