...
| 8 | `base_url` = `/dev/<ns>/<env>/` | HTTPRoute forwards the prefix **unchanged** (no URLRewrite filter); the controller injects the prefix into the container as `NOTEBOOK_ARGS=--ServerApp.base_url=<prefix>` (`::withNotebookBaseURL`), merging into a user-declared `NOTEBOOK_ARGS` rather than clobbering it | Jupyter must serve under that prefix via `ServerApp.base_url`, which the injected flag supplies | ✅ Gap C closed: the controller injects the flag — a base_url the environment declares itself wins, and an entry fed by `valueFrom` is left alone |
...
published one cannot drift. An environment that names its own `base_url`, or whose `NOTEBOOK_ARGS` is
fed by `valueFrom` (unreadable while reconciling), keeps its declaration.
...
| C injecting jupyter `base_url` | controller | **Closed controller-side**: `::withNotebookBaseURL` injects `NOTEBOOK_ARGS=--ServerApp.base_url=/dev/<ns>/<env>/` (merge, don't clobber a user value) — the stock launcher honours it, so no image change | — |
